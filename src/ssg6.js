define(function (require, exports, module) {
    ...
    for (var i = 0; i < vertices.length; i++) {
        ...
        ball.state.setTransform(+Transform.rotateZ(Math.PI), {
            curve: 'linear',
            duration: 10000
        });
    ...
    /*Timer.every(function () {
        console.log(balls[0].state.getTransform());
        for (var i = 0; i < balls.length; i++) {
            balls[i].state.resetTransform();
            balls[i].state.setTransform(Transform.rotateZ(Math.PI), {
                curve: 'linear',
                duration: 10000
            });
        }
    ...
    logo.state = new StateModifier({
        origin: [0.5, 0.5]
    });
    layout.content.add(logo.state).add(particle).add(logo);
