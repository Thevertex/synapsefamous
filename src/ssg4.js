define(function (require, exports, module) {
    ...
    Engine.on('prerender', function(){
        for( var i=0; i<balls.length; i++ ){
            //balls[i].state.halt();
            balls[i].state.setTransform(Transform.rotateY(Math.random()*Math.PI/2), { curve: 'easeOut', duration: 500 });
            balls[i].state.setTransform(Transform.rotateZ(Math.random()*Math.PI/2), { curve: 'easeOut', duration: 500 });
            balls[i].state.setTransform(Transform.rotateX(Math.random()*Math.PI/2), { curve: 'easeOut', duration: 500 });
            balls[i].particle.applyForce(new Vector(0, 0, -0.005 * 10));
        }
    });
